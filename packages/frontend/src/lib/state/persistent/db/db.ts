		...
		const db = await openDB<OpenPlebDB>(DB_NAME, DB_VERSION, {
			upgrade: (db, oldVersion) => {
				if (!oldVersion) {
					...
					db.createObjectStore('encrypted-sessions');
				}
				if (oldVersion < 2) {
				...
				if (oldVersion < 3) {
					if (!db.objectStoreNames.contains('encrypted-sessions')) {
						db.createObjectStore('encrypted-sessions');
					}
				}
				if (oldVersion < 4) {
					if (!db.objectStoreNames.contains('encrypted-offers')) {
						db.createObjectStore('encrypted-offers');
					}
				}
				if (oldVersion < 5) {
					if (!db.objectStoreNames.contains('encrypted-providers')) {
						db.createObjectStore('encrypted-providers');
					}
				}
			},
			blocked: () => {
