...
export const claimOffer = async (offerId: string, pubkey: string, bond: string) => {
	if (!pubkey || !offerId) {
	...
	if (offer.status === OFFER_STATE.INVOICE_PAID) {
		...
		const tokenAmount = token.proofs.reduce((acc, proof) => acc+proof.amount, 0)
		...
		try {
			...
			return new Response(`Error redeeming bond: ${err.message}`, { status: 400 });
		}
