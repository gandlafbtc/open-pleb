...
//   }
export const resolveDispute = async (body: {
	offerId: number;
	resolution: string;
	...
	const [offer] = await db
		.select()
		.from(offerTable)
		.where(eq(offerTable.id, body.offerId));
	...
	switch (body.resolution) {
		case RESOLUTION_PATHS.RETURN: {
			...
			await handlePayouts(offer, OFFER_STATE.RESOLVED, {
				maker,
				taker,
				resolutionReason: `${RESOLUTION_PATHS.RETURN}: ${body.resolutionReason}`,
			});
			break;
		}
		case RESOLUTION_PATHS.MAKER_WINS: {
			...
			await handlePayouts(offer, OFFER_STATE.RESOLVED, {
				maker,
				taker: 0,
				resolutionReason: `${RESOLUTION_PATHS.MAKER_WINS}: ${body.resolutionReason}`,
			});
			break;
		}
		case RESOLUTION_PATHS.TAKER_WINS: {
			...
			await handlePayouts(offer, OFFER_STATE.RESOLVED, {
				maker: 0,
				taker,
				resolutionReason: `${RESOLUTION_PATHS.TAKER_WINS}: ${body.resolutionReason}`,
			});
			break;
		}
		case RESOLUTION_PATHS.SPLIT: {
			...
			await handlePayouts(offer, OFFER_STATE.RESOLVED, {
				maker: half,
				taker: half,
				resolutionReason: `${RESOLUTION_PATHS.SPLIT}: ${body.resolutionReason}`,
			});
			break;
		}
		default:
