...
export const subsAPI =	new Elysia().get("/vapid", async ({set}) => {
		try {
			const publicKey = encodeBase64Url(
				await crypto.subtle.exportKey("raw", VapidKeys.getInstance().vapidKeys.publicKey),
			);
			return { publicKey };
		} catch (error) {
			set.status = 400
	...
	.post("/subscribe", async ({ body, set }) => {
		try {
			...
			const subscriber = VapidKeys.getInstance().appServer.subscribe(subscription);
